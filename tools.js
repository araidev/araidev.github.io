...
function renderTrash() {
    ...
    // Urutkan berdasarkan waktu hapus terbaru
    keys.sort((a, b) => (trashCache[b].deletedAt || 0) - (trashCache[a].deletedAt || 0));

    keys.forEach(k => {
        const data = trashCache[k];
        const isExpired = (now - data.deletedAt) > 86400000; // 24 Jam (ms)
        ...
        // Susun teks keterangan sisa (desc & price)
    ...
    // Nyalakan/matikan titik notifikasi merah di toolbar
    if(btnPengingat) {
